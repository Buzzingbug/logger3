...
export const guildLogRoutes = pgTable("guild_log_routes", {
...
export const ignoredEntities = pgTable("ignored_entities", {
  id: uuid("id").defaultRandom().primaryKey(),
  guildId: text("guild_id").notNull().references(() => guilds.id, { onDelete: "cascade" }),
  ...
  ignoredUniqueIdx: uniqueIndex("ignored_entities_unique_idx").on(table.guildId, table.entityType, table.entityId)
}));
...
export const logEvents = pgTable("log_events", {
  ...
  guildTypeCreatedIdx: index("log_events_guild_type_created_idx").on(table.guildId, table.type, table.createdAt),
  guildActorCreatedIdx: index("log_events_guild_actor_created_idx").on(table.guildId, table.actorId, table.createdAt),
  guildTargetCreatedIdx: index("log_events_guild_target_created_idx").on(table.guildId, table.targetId, table.createdAt)
}));
...
export const messageSnapshots = pgTable("message_snapshots", {
  ...
  snapshotGuildExpiresIdx: index("message_snapshots_guild_expires_idx").on(table.guildId, table.expiresAt)
}));

export const deliveryJobs = pgTable("delivery_jobs", {
  id: uuid("id").defaultRandom().primaryKey(),
  logEventId: uuid("log_event_id").notNull().references(() => logEvents.id, { onDelete: "cascade" }),
